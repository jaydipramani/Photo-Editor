...
public abstract class StraightCollageLayout implements PolishLayout {
    ...
    private int color = -1;
    private List<PolishLine> lines = new ArrayList();
    private StraightArea outerArea;
    private List<PolishLine> outerLines = new ArrayList(4);
    private float padding;
    private float radian;
    private ArrayList<PolishLayout.Step> steps = new ArrayList<>();
    ...
    public ArrayList<PolishLayout.Step> getSteps() {
        return this.steps;
    }
    ...
    @Override 
    public void update() {
        for (PolishLine update : this.lines) {
    ...
    @Override 
    public PolishArea getArea(int i) {
    ...
    public void addLine(int i, PolishLine.Direction direction, float f) {
        ...
        PolishLayout.Step step = new PolishLayout.Step();
        int i2 = 0;
        step.type = 0;
    ...
    private void updateLineLimit() {
        for (int i = 0; i < this.lines.size(); i++) {
            PolishLine line = this.lines.get(i);
            updateUpperLine(line);
            updateLowerLine(line);
        }
    }

    private void updateLowerLine(PolishLine line) {
        for (int i = 0; i < this.lines.size(); i++) {
            PolishLine line2 = this.lines.get(i);
            if (line2 != line && line2.direction() == line.direction()) {
                if (line2.direction() == PolishLine.Direction.HORIZONTAL) {
    ...
    private void updateUpperLine(PolishLine line) {
        for (int i = 0; i < this.lines.size(); i++) {
            PolishLine line2 = this.lines.get(i);
            if (line2 != line && line2.direction() == line.direction()) {
                if (line2.direction() == PolishLine.Direction.HORIZONTAL) {
    ...
    @Override 
    public PolishLayout.Info generateInfo() {
        PolishLayout.Info info = new PolishLayout.Info();
        info.type = 0;
        info.padding = this.padding;
        info.radian = this.radian;
        info.color = this.color;
        info.steps = this.steps;
        ArrayList<PolishLayout.LineInfo> arrayList = new ArrayList<>();
        for (PolishLine lineInfo : this.lines) {
            arrayList.add(new PolishLayout.LineInfo(lineInfo));
        }
