...
public class ScaleGestureDetector {
            ...
            if (actionMasked == 1) {
                ...
                if (pointerCount > 2) {
                    ...
                    if (pointerId == i) {
                        ...
                        if (pointerId == this.mActiveId1 && (findNewActiveIndex2 = findNewActiveIndex(motionEvent, i, actionIndex)) >= 0) {
                            this.mListener.onScaleEnd(view, this);
    ...
    /* JADX INFO: Multiple debug info for r2v5 float: [D('motionEvent2' android.view.MotionEvent), D('x4' float)] */
    /* JADX INFO: Multiple debug info for r8v4 float: [D('y4' float), D('x' float)] */
    private void setContext(View view, MotionEvent motionEvent) {
        ...
        if (findPointerIndex >= 0 && findPointerIndex2 >= 0 && findPointerIndex3 >= 0) {
            if (findPointerIndex4 >= 0) {
                ...
                float x5 = motionEvent.getY(findPointerIndex4) - y3;
                this.mCurrSpanVector.set(x4, x5);
                this.mPrevFingerDiffX = f;
                this.mPrevFingerDiffY = y2 - y;
                this.mCurrFingerDiffX = x4;
                this.mCurrFingerDiffY = x5;
                this.mFocusX = (x4 * 0.5f) + x3;
                this.mFocusY = (0.5f * x5) + y3;
