...
public class PolishGrid {
    ...
    public PolishGrid(Drawable drawable2, PolishArea area2, Matrix matrix2) {
    ...
    private void animateTranslate(final View view, final float f, final float f2) {
        ...
        this.animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator valueAnimator) {
                PolishGrid.this.translate(f * ((Float) valueAnimator.getAnimatedValue()).floatValue(), f2 * ((Float) valueAnimator.getAnimatedValue()).floatValue());
                view.invalidate();
            }
        });
    ...
    public void fillArea(final View view, boolean z) {
        ...
        if (!isFilledArea()) {
            ...
            float left2 = rectF.left > this.area.left() ? this.area.left() - rectF.left : 0.0f;
            ...
            final float bottom = rectF.bottom < this.area.bottom() ? this.area.bottom() - rectF.bottom : f2;
            this.animator.end();
            this.animator.removeAllUpdateListeners();
            this.animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                ...
                public void onAnimationUpdate(ValueAnimator valueAnimator) {
                    ...
                    PolishGrid.this.zoom(f2, f2, pointF);
                    PolishGrid.this.postTranslate(left * floatValue, bottom * floatValue);
                    view.invalidate();
                }
            });
            if (z) {
    ...
    public void updateWith(MotionEvent motionEvent, PolishLine line) {
        ...
        if (line.direction() == PolishLine.Direction.HORIZONTAL) {
            translate(0.0f, y);
        } else if (line.direction() == PolishLine.Direction.VERTICAL) {
            translate(x, 0.0f);
        }
        RectF currentDrawableBounds = getCurrentDrawableBounds();
        PolishArea area3 = getArea();
    ...
    public void setArea(PolishArea area2) {
        this.area = area2;
    }
