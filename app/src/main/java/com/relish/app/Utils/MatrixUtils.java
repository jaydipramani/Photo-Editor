...
public class MatrixUtils {
    ...
    public static float getMinMatrixScale(PolishGrid puzzlePiece) {
        if (puzzlePiece == null) {
            return 1.0f;
        }
    ...
    public static boolean judgeIsImageContainsBorder(PolishGrid puzzlePiece, float f) {
        Matrix matrix = sTempMatrix;
        matrix.reset();
        matrix.setRotate(-f);
    ...
    public static float[] calculateImageIndents(PolishGrid puzzlePiece) {
        if (puzzlePiece == null) {
    ...
    public static Matrix generateMatrix(PolishGrid puzzlePiece, float f) {
        return generateMatrix(puzzlePiece.getArea(), puzzlePiece.getDrawable(), f);
    }

    public static Matrix generateMatrix(PolishArea area, Drawable drawable, float f) {
        return generateCenterCropMatrix(area, drawable.getIntrinsicWidth(), drawable.getIntrinsicHeight(), f);
    }

    private static Matrix generateCenterCropMatrix(PolishArea area, int i, int i2, float f) {
        float f2;
